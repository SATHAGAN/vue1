<template>
  <div class="a1">
    <div class="a2">
      <img
        src="data:image/jpeg;base64,"
        alt="hello"
      />
    </div>
    <div class="a3">
      <h3>Why Choose Us</h3>

      <b>1.Traveller's Choice</b>
      <p>
        “Explore In Lanka is your ultimate travel partner in Sri Lanka, offering
        unforgettable experiences for adventurers and wanderers from around the
        globe. Discover the magic of Sri Lanka with expert guidance, seamless
        service, and journeys tailored just for you!”
      </p>

      <b>2.Passionate Guides</b>
      <p>
        Our expert guides are passionate storytellers, sharing the rich history,
        culture, and natural beauty of Sri Lanka with enthusiasm and
        authenticity.
      </p>

      <b>3.Tailored Experiences</b>
      <p>
        Every traveler is unique, and so are our tours. Whether you’re seeking
        adventure, relaxation, or cultural immersion, we customize every detail
        to match your preferences and interests.
      </p>

      <b>4.Passionate Guides</b>
      <p>
        We believe in providing exceptional experiences at competitive prices.
        With us, you get premium service and unforgettable adventures without
        breaking the bank.
      </p>

      <b>5.Unbeatable Value</b>
      <p>
        Our expert guides are passionate storytellers, sharing the rich history,
        culture, and natural beauty of Sri Lanka with enthusiasm and
        authenticity.
      </p>
      <div class="qut">
        <a>&quot;</a>
        <p>
          Your Journey, Our DesignEmbark on a journey with us and discover the
          enchanting beauty of Sri Lanka like never before. Let us turn your
          dream vacation into reality!
        </p>
        <h2 class="cen">Most Popular Tours</h2>
        <br />
        <h2>Itinerates</h2>
      </div>
      <div class="scroll"></div>
    </div>
  </div>
</template>
<script>
export default {
  name: "HomeView",
};
</script>

<style scoped>
.a1 {
  background-color: deepskyblue;
  padding-top: 200px;
}
.a2 {
  display: flex;
  justify-content: center;
}
.a3 p {
  text-align: center;
}
.a3 b {
  font-size: 30px;
  text-align: right;
}
.qut {
  padding: 70px;
}
.qut a {
  font-size: 100px;
  font-style: italic;
  color: yellow;
  padding: 0px;
}
.qut h2 {
  color: aliceblue;
  font-family: sans-serif;
}
.cen {
  text-align: center;
  padding-top: 30px;
}
</style>
